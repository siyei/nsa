...
class Router {
	...
	private $reserved_keys = array('carpeta', 'controller', 'action');
	...
	public function apply($full_url) {
        ...
        if(empty($mapped_path['carpeta'])) $mapped_path['carpeta'] = '';
        
		$mapped_url = array(
			'controller' => $mapped_path['controller'], 
			'action'    => $mapped_path['action'], 
            'carpeta'   => $mapped_path['carpeta'],
			'params'    => $url['query']
            //'carpetax' => $mapped_path['carpetaxs']
		);
